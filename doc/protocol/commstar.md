...
| Handheld-to-host RECORD transfer | **Provisional** | Works with controlled content: an application nominates a counted buffer `[u8 count][payload]` and the payload reaches the host verbatim, wrapped as `c3 03 01 · 1e · payload · 1c`. The wrapper bytes, the intermediate state `0062`, and clean teardown are open |
