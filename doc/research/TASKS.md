...
1. **Emulator demonstration of the handshake gating:** model "LINK_STATUS bit 6 never clears" and show the stock firmware never reaches the `2FAE` RX-enable; note this validates firmware-side control flow only (the controller model is synthetic).
2. **Keep the existing static backlog** (session-module RECORD-vs-BLOCK mapping, runtime loader `ram:D370`, guarded structural repairs, deferred final annotation sweep).
