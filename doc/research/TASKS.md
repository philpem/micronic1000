...
2. **Phase 2:** receive-convention sweep with the witness ROM (`2E3E`, now enables `LINK_CTRL` 6/7) + the Arduino `RX_SWEEP` mode — stimulus must arrive **after** the ~10 ms TX transaction (enabled window); a burst-timed reply (~1–9 ms) lands in the disabled window.
...
* **Finding 2 — temporal gating (CONFIRMED):** gating is temporal, not permanent. `LINK_CTRL` 6/7 cleared for the whole `LinkBlockTx` transaction and restored by `34BD` at `2FAE` after it, on success or `0EEh` timeout. Disabled window is the transaction itself: bit-6 wait is 620×59 T ~=9.92 ms at 3.6864 MHz (`ROM00:32F0`), within ~93.75 ms retry, so RX disabled ~10–12 ms then enabled for remainder.
* **Correction to re-framing (CONFIRMED):** `conn3`–`conn13` Arduino replies were sent ~1–9 ms after the handheld's burst, i.e. inside the disabled ~10–12 ms window, so framed data could not be received; the optical reaction was a front-end/cadence effect, not a framed receive. "Receive window only opens after the transmit transaction completes" remains correct; sharpened: receiver IS re-enabled after each attempt, replies simply landed in disabled window. Discard any wording implying RX was never enabled.
