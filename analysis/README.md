...
* `feedback_scope.py` measures a Keysight `x-axis,D0-D7` CSV from one feedback
  ...
  the byte sampled at the candidate clock edges. The trial-5 source capture
  is tracked at `analysis/captures/feedback-trial5-keysight.csv`; run
  `python3 analysis/feedback_scope.py analysis/captures/feedback-trial5-keysight.csv`.
...
- `--watch-read LO:HI[,...]` — every memory **read** landing in an inclusive hex range, reported with the address, the value, the reading PC, `SP` and the current bank. Mirrors `--watch-mem` but hooks reads (the PC reported is the address of the instruction doing the read, not pc-after). Per-range print cap via `--watch-read-limit` (default 24); totals at exit include distinct reading PCs and address extent. Used to locate the reader of a data table that has no static xref — e.g. the session-object callback table via `UI_FormExitDispatchNext` (`ROM01:06D3`-`0720`) reading the 5-entry pointer table at `ram:D081` (double indirection via `ram:D12F` slots).
