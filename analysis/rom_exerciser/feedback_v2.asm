...
; Candidate stock helper address from the v2 protocol proposal. Its hardware
; effect remains subject to the parent Commstar audit.
LinkClear67      equ 0x34D2
...
S_PREVIEW        equ STATE+15          ; RX preview or v2 fast-sample summary
...
guard50:         call tick5
                ...
                ld a,(S_MODE)
                cp 5
                jr c,start_marker_kept
                call yellow_release   ; H/J/K emit a 2 ms falling-edge marker
start_marker_kept:
                ld a,(S_MODE)
                cp 1
                jr nz,trial_rx
...
trial_g:        ld a,(S_MODE)
                ...
                call run_pending_rx
trial_done:
                ; A fixed 100 ms post-trial cooldown precedes UART feedback.
                ; H/J/K released the marker after tick2; legacy modes hold
                ; it low until restore_gate. The delay separates feedback.
                ld b,20
...
; H and J share one bounded sampler and always execute the complete window.
; The eight v2 summary bytes are OR, AND, first bit-4 index, first bit-0
; index, and the 16-bit sample count. Indexes are little-endian; FFFF means
; no sample asserted that bit. H/J run 1000 samples; K repeats the watcher
; cycle for 600 samples. Z80-cycle audit (all values include the sample read,
; aggregate, index checks/stores, counter and loop branch): H/J first sample
; 389 T; 766 lower-counter samples at 317 T; 232 upper-counter samples at
; 335 T; final sample 330 T = 321,261 T. Budget 340,000 T (92.23 ms).
; K: first sample 578 T; 510 lower-counter samples at 542 T; 88 upper
; samples at 567 T; final sample 574 T = 327,468 T. Budget 360,000 T
; (97.66 ms). Both budgets are below 100 ms at 3.6864 MHz.
; Per-sample H/J path: 133 T prefix; set-bit index path 83 T on first hit,
; 47 T thereafter (both bits set); 90 T counter/mode/lower limit, or 108 T
; upper limit. K lower repeat = 27+115+171+17+60+20+47+35+27+23=542 T;
; first-hit index adds 36 T, upper counter limit adds 25 T, and final limit
; adds 32 T. CALL 34D2 and CALL 34BD each cost 115 T including their full
; 98 T helper bodies; pending skips the rearm call.
; K tests bit 4 on the same full-byte read stored for summary, records any
; hit in S_P4, and omits the stock RX dispatch even when pending is set.
run_fast_h:      call link_reset_select
                call LinkFinish
                jp fast_sample
run_fast_j:      call link_reset_select
                call LinkClear67
                jp fast_sample
run_fast_k:      call link_reset_select
                xor a
                ld (S_P4),a           ; K: no pending test seen yet
                jp fast_sample
fast_sample:    in a,(LINK_STAT)
                ld (S_BEFORE),a
                ld a,8
                ld (S_PREVLEN),a
                ld a,0xFF
...
fast_loop:      ld a,(S_MODE)
                ...
                ld (S_P4),a
                jr fast_sample_byte
fast_k_clear:   call LinkFinish
                jr fast_sample_byte
fast_read:      in a,(LINK_STAT)
                ld (S_RAW2D),a
fast_sample_byte:
                ld a,(S_RAW2D)
                ld b,a
...
fast_count:     inc ix
                ...
                jr nz,fast_loop
                jr fast_done
fast_limit_hj:  ld a,h
                cp 0x03
                jr nz,fast_loop
                ld a,l
                cp 0xE8
                jr nz,fast_loop
fast_done:      call LinkFinish
                ld (S_PREVIEW+6),ix
                in a,(LINK_STAT)
                ld (S_AFTER),a
                ret
...
banner:          db 'IR FEEDBACK V2 W..K',0
