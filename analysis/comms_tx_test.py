#!/usr/bin/env python3
"""comms_tx_test.py - verify the comms module against the real
firmware TX path.
...
capture every 4Dh write and compare it against the reusable
micronic.proto model - proving the protocol description matches the
...
PAYLOAD = bytes([0x04, 0x44, 0x00, 0x41, 0x42, 0x43, 0x44])  # type=4,cmd=4400,data ABCD
...
def ich(*a):
    ...
    if p == 0x4B: return 0x80      # TX empty always
...
# run until it emits a frame or we bound it
import time
t0=time.time()
...
print("expected payload:", [f"{b:02X}" for b in PAYLOAD])
