...
def run_decoder(code, widths):
    """Run an assembled decoder over a list of element widths.

    Sets up the parameter block exactly as the firmware does, runs until
    the hook returns to a sentinel, and reads back whatever it published.
    """
...
@pytest.fixture
def decode(decoder):
    """Decode with the default build."""
    return lambda widths: run_decoder(decoder, widths)
