...
decodes **Code 39**, **UPC-A** and **EAN-13**, in either scan direction.
...
A wand can be drawn right-to-left, and the symbol is structurally
symmetric — guard, six, centre, six, guard — so a reversed scan is a
forward scan with the element list backwards. The decoder tries forward,
and on failure reverses into a scratch buffer and tries once more. The
cost is one extra pass on symbols that fail forward, which includes every
genuinely reversed scan and every piece of noise.

### Not handled

* **UPC-E**, the zero-suppressed six-digit form. It is 33 modules, not 95,
  so it needs its own element count and expansion rules.
* **Code 128, Interleaved 2 of 5, Codabar.** Nothing about the hook
  prevents them.
...
src/upc.asm            EAN-13 and UPC-A, either direction
...
tools/upc.py           EAN-13 / UPC-A encoder, for generating test scans
